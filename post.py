...
if not HF_TOKEN:  raise RuntimeError("Missing HF_TOKEN")
if not STABILITY_KEY:  raise RuntimeError("Missing STABILITY_API_KEY")
...
def generate_verse_and_reflection():
    ...
    if not verse.startswith(("“", "\"")):
        verse = f"“{verse.strip('\"').strip('“').strip()}”"
...
def generate_image_bytes(prompt, width=1024, height=1024):
    ...
    body = {"text_prompts": [{"text": prompt}],
            "cfg_scale": 7, "width": width, "height": height, "samples": 1}
...
if __name__ == "__main__":
    ...
    img_bytes = generate_image_bytes(f"{THEME}, {STYLE}, instagram composition, detailed, high quality")
    ...
    json.dump(payload, open("post_payload.json","w",encoding="utf-8"), ensure_ascii=False, indent=2)
